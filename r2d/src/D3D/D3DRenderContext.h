...
namespace r2d {
	...
	public:
		...
		virtual void RenderPoint(vec2 point, ivec3 color = DEFAULT_COLOR_RGB, float pointSize = 1.0f) const;
		virtual void RenderLine(vec2 point1, vec2 point2, ivec3 color = DEFAULT_COLOR_RGB) const;
		virtual void RenderWireRectangle(Rect geometry, ivec3 color = DEFAULT_COLOR_RGB) const;
		virtual void RenderSolidRectangle(Rect geometry, ivec3 color = DEFAULT_COLOR_RGB) const;
		virtual void RenderWireCircle(const Circle& c, ivec3 color = DEFAULT_COLOR_RGB) const;
		virtual void RenderSolidCircle(const Circle& c, ivec3 color = DEFAULT_COLOR_RGB) const;
		virtual void RenderWirePolygon(const Polygon2d& p, ivec3 color = DEFAULT_COLOR_RGB) const;
		virtual void RenderSolidPolygon(const Polygon2d& p, ivec3 color = DEFAULT_COLOR_RGB) const;
		virtual void RenderWorldAxises(const vec2& position, float angle) const;

		// Custom primitives render routines
		virtual void AddToRenderingQueue(const IRenderable* pRenderableObject);
